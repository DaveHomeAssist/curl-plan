...
| `FR-A11Y` | Small screen and accessibility reachability | Complete primary loops on constrained screens | SwiftUI layout and accessibility identifiers | Core controls remain reachable on a small device | Scrollable surfaces expose actions without overlap or hidden dead ends | Medium | Allowed: reachable local controls. Banned: hidden debug-only paths as proof. | [Simulator journey run](https://github.com/DaveHomeAssist/curl-plan/actions/runs/35845736958) passed on standard and small devices; manual VoiceOver, Dynamic Type, and target review remain separate | Yellow: tested demo journey passed; assistive checks pending | Required for toolbar, sheet, button grid, tab, card, dynamic type changes |
...
| `ios/CurlPlan/AuthView.swift` | `FR-SETUP`, `FR-ACCOUNT`, `FR-A11Y`, `FR-CLAIMS` |
...
| `ios/CurlPlanUITests/CurlPlanJourneyUITests.swift` | `test-support`, `FR-A11Y`, `FR-CLAIMS` |
