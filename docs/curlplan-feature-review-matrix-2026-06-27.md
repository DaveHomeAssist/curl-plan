...
| `FR-CLAIMS` | Unsupported authority guardrails | Avoid claims the local app cannot prove | Static visible text and claim ledger | App does not claim public roster, official result, GPS verification, here now, likes, or comments | Unsupported wording is removed or backed by real authority before ship | High | Allowed: explicitly local claims. Banned: public, official, GPS, here now, likes, comments, remote social authority. | `rg -in "likes|comments|public roster|here now|GPS verification|official result" ios/CurlPlan ios/CurlPlanUITests tests/CurlPlanCoreTests -g '*.swift'`; visible text claim scan | Green | Required for any copy, feed, profile, map, roster, result, sharing language change |
...
| `Package.swift` | `build-support`, all model-backed feature rows |
