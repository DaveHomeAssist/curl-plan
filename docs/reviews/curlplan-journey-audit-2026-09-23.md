...
**Web: Red. iOS: Grey.** The live web journeys mostly work, but browser Back leaves both apps and the current homepage gives users no route to the original Classic planner. The native screenflow cannot be signed off from this host because full Xcode and an iOS simulator are absent. This is an audit finding, not a claim that the iOS app fails.

No product code was changed during this audit. Failed journeys have not been repaired or successfully retested.

## Scope and method

- Source: `main` at `623bd18fb2da2c5ebdb5d6af5179c3039dda73b6`, clean before audit. The current [Verify run](https://github.com/DaveHomeAssist/curl-plan/actions/runs/35842151117), [security run](https://github.com/DaveHomeAssist/curl-plan/actions/runs/35842151080), and [Pages deployment](https://github.com/DaveHomeAssist/curl-plan/actions/runs/35842149973) succeeded for that SHA.
...
- Limits: Chrome was the only browser driven interactively. Safari, VoiceOver, recruited curler usability sessions, and native iOS simulator runs remain unverified.
...
| I1 | **Unverified** | Current checkout has no `ios/CurlPlanUITests/` target. This host selects `/Library/Developer/CommandLineTools`; `simctl` is unavailable and there is no `Xcode.app`. CI build/unit success does not prove interactive navigation or visual layout. |
...
2. Run I1 on small and standard iOS simulators with full Xcode: clean demo entry, all four tabs, Stop/Profile push and back, relaunch persistence, layout, Dynamic Type, and VoiceOver. Record simulator model/runtime, build SHA, screenshots, and any console warnings.
